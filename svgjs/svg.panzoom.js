...
SVG.extend(SVG.Doc, SVG.Nested, {
  ...
  panZoom: function(options) {
	...
	var panButton = options.panButton || 3 // added
    ...
    var panStart = function(ev) {
	  // added
      if((ev.which || ev.buttons) != panButton){
          return
      }
	  // end added
      ...
      lastP = initP = {x: lastTouches[0].clientX, y: lastTouches[0].clientY }
	...
	//krumble
	this.on('contextmenu.panZoom', function(ev){
		if(panButton != 3)
			return;
		var currentTouches = normalizeEvent(ev);
  ...
  zoom: function(level, point) {
    var style = window.getComputedStyle(this.doc().node) // change from this.node to this.doc().node
    ...
    if(this.fire('zoom', {box: box, focus: point, level: level}).event().defaultPrevented) // <- add level property
      return this
