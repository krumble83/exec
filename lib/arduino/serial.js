...
ctx.registerNode('available', {
	...
	tooltip: 'Get the number of bytes (characters) available for reading from the serial port. This is data that’s already arrived and stored in the serial receive buffer (which holds 64 bytes). available() inherits from the Stream utility class.'
});
...
ctx.registerNode('begin', {
	import: 'arduino.base.base',
	categories: ['Arduino/Serial'],
	keywords: 'serial connect,serial.begin,begin serial',
	title:'Serial.Begin()',
	inputs: [
		{id: 'entry', type: 'core.exec'},
		{id: 'port', type: 'arduino.serial.port', label: 'Port', tooltip: 'Arduino Serial Port Device to connect to'},
		{id: 'bauds', type: 'core.type.int'},
		{id: 'wait', type: 'core.type.bool', tooltip: 'Wait for connection available before continue execution'}
	],
	outputs: [
		{id: 'exit', type: 'core.exec'},
		{id: 'connection', type: 'arduino.serial.connection'}
	],
	tooltip: 'Sets the data rate in bits per second (baud) for serial data transmission.'
});
