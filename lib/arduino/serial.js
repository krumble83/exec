;exLIB.package('arduino.io', function(pack){
	...
	pack.Type('port', 'Arduino Serial Port Device').Inherits('core.object');

	
	var n = pack.Node('available', 'available()')
	...
	n = pack.Node('begin', 'begin() (Serial)')
	...
	n = pack.Node('writeByte', 'Write Byte')
		.Import('tpl.node.function')
		.MakeEntry().MakeExit()
